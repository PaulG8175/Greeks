...
#Q2)
#Calculer le prix P du call, P= e^{-rT}E[(S(T)-K)+] en utilisant np.mean
...
#Calculer le prix P du call, P= e^{-rT}E[(S(T)-K)+] en utilisant la formule de l'integrale
...
#Q3) Rappeller les définitions des grecques ∆, Γ et Vega. Les calculer 
...
#question4-5)
#par méthode par différences finies
# On fixe un Z commun pour réduire la variance (variables communes)
Z = np.random.normal(0, 1, N_simulations)
...
# Vega : on fait varier sigma, pas S0
epsi_sig = 0.001
...
#question6)
#méthode trajectorielle avec méthode antithétique 
...
for N in N_values:
    ...
    for greek_name, evals in evaluations:
        for i in range(4):
            m = np.mean(evals[i])
            n_size = N_demi if i in (2, 3) else N
...
#q7 — Grecques vanille en fonction de S0 in [0, 2]
print("=" * 80)
...
# question 8 — parametres du sujet : alpha=beta=0.5, sigma=(0.25,0.30)
...
# q10 — grecques en fonction de S1,0
N_ref  = 50000
...
for s10v in S10_vals:
    ...
    # Comparaison : alpha * call sur un actif S1 de strike K_eff = K - beta*S2_0
    K_eff = K_ - be * S20
    if K_eff > 0:
...
# Valeurs ATM (K=1 → index le plus proche)
...
# q12 — grecques en fonction de rho
...
# q13 — Relation Call/Put par parité (analytique)
print("=" * 80)
...
gamma1_put_mc  = np.exp(-r_*T_) * np.mean(dirac_p * (al*S1p/S10)**2)
...
# q14 — Delta-hedging quotidien : paramètres clés
print("=" * 80)
